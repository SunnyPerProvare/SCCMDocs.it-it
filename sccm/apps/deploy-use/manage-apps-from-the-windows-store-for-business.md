...
Configuration Manager supporta la gestione delle app di Microsoft Store per le aziende su dispositivi Windows 10 che eseguono il client di Configuration Manager e anche su dispositivi Windows 10 registrati in Microsoft Intune (configurazione ibrida). Configuration Manager offre le funzionalità seguenti per le app online e offline.
...
- I PC devono essere aggiunti all'area di lavoro di Azure Active Directory e devono trovarsi nello stesso tenant di AAD in cui Microsoft Store per le aziende è stato registrato come strumento di gestione.
...
- Quando l'installazione è imposta dall'utente che installa l'applicazione, dall'applicazione che ha raggiunto il termine di scadenza dell'installazione o dalla rivalutazione post-installazione per le distribuzioni richieste:
    ...
    - Lo stato dell'applicazione nella console di Configuration Manager indica l'esito negativo con l'errore "L'app di Microsoft Store è stata aperta nel computer client ed è in attesa del completamento dell'installazione da parte dell'utente".
...
- Se sono stati distribuiti criteri di gruppo che disabilitano l'accesso alla versione consumer di Microsoft Store, le distribuzioni di Microsoft Store per le aziende non funzionano, anche se Microsoft Store per le aziende è abilitato.
...
4. Al termine viene visualizzato l'account configurato nell'elenco di **Microsoft Store per le aziende** nella console di Configuration Manager.
...
6. Controllare la pagina **Informazioni** ed eseguire gli eventuali passaggi aggiuntivi e le configurazioni indicati. Queste configurazioni sono necessarie per usare il servizio con Configuration Manager. Ad esempio, per configurare Microsoft Store per le aziende:
    ...
    - Nella console di Microsoft Store per le aziende è necessario configurare Configuration Manager come strumento di gestione dello Store, abilitare il supporto per le app con licenza offline e acquistare almeno un'app. 
...
Viene creata un'applicazione di Configuration Manager contenente l'app di Microsoft Store per le aziende. Sarà quindi possibile distribuire e monitorare l'applicazione come qualsiasi altra applicazione di Configuration Manager.
...
Dopo aver distribuito le app online, si noti che tutte le app vengono aggiornate direttamente da Microsoft Store.

Quando vengono distribuite app offline in dispositivi Windows 10 usando il client di Configuration Manager, ad esempio in ambienti multiutente come le classi, disabilitare Microsoft Store e/o gli aggiornamenti dell'applicazione tramite Microsoft Store che usano ad esempio i [criteri di gruppo](https://docs.microsoft.com/en-us/windows/configuration/stop-employees-from-using-microsoft-store#a-href-idblock-store-group-policyablock-microsoft-store-using-group-policy). In questo modo gli utenti non potranno aggiornare le applicazioni esterne alle distribuzioni di Configuration Manager. Dopo che l'amministratore di Microsoft Store per le aziende ha acquistato l'app, mettendola così a disposizione nello stato offline per la sincronizzazione con Configuration Manager, non pubblicare l'app agli utenti affinché possano installare o eseguire l'aggiornamento online. In questo modo tutti gli utenti riceveranno solo gli aggiornamenti delle app tramite Configuration Manager. 
