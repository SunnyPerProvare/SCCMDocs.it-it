...
description: Informazioni su come creare e distribuire i criteri di conformità del dispositivo in Configuration Manager.
ms.date: 03/22/2018
...
4. Nella pagina **Generale** della Creazione guidata criteri di conformità specificare le informazioni seguenti:  
    ...
    - **Descrizione**: inserire una descrizione che offra una panoramica del profilo VPN e altre informazioni rilevanti per facilitarne l'identificazione nella console di Configuration Manager. Non può superare i 256 caratteri.  

    - **Tipo di criteri di conformità**: selezionare il tipo di criteri che si vuole creare, a seconda che il dispositivo sia gestito da Configuration Manager.  
...
6. Nella pagina **Regole** , indicare una o più regole che definiscono la configurazione che i dispositivi devono possedere per essere ritenuti conformi. Quando si creano criteri di conformità, alcune regole sono abilitate per impostazione predefinita, ma è possibile modificarle o eliminarle. Per un elenco completo di tutte le regole, vedere la sezione "Regole dei criteri di conformità" più avanti in questo articolo.  
    ...
    > [!NOTE]  
    > Nei PC Windows la versione 8,1 di Windows viene segnalata come 6,3. Se la regola della versione del sistema operativo è impostata su Windows 8.1 per Windows, il dispositivo risulterà non conforme anche se il sistema operativo installato è Windows 8.1. Assicurarsi di impostare la versione di Windows *riportata* corretta per le regole della versione minima e massima del sistema operativo. Il numero di versione deve corrispondere alla versione restituita dal comando **winver**. Windows Mobile non presenta questo problema; la versione viene segnalata come 8,1 come previsto.  
    > 
    > Per i PC Windows con Windows 10, la versione deve essere impostata su **10,0** più il numero di build del sistema operativo restituito dal comando **WINVER** .  
...
4. È possibile esaminare le informazioni di riepilogo sulla conformità della distribuzione del criterio nella pagina principale. Per visualizzare informazioni più dettagliate, selezionare la distribuzione. Nel gruppo **Distribuzione** della scheda **Home** scegliere **Visualizza stato** per aprire la pagina **Stato distribuzione**.  
    ...
    - **Sconosciuto**: Visualizza un elenco di tutti gli utenti e i dispositivi che non sono conformi alla distribuzione dei criteri selezionata, insieme allo stato client corrente dei dispositivi.  
...
3. Per abilitare altre colonne, fare clic con il pulsante destro del mouse su una delle colonne. È possibile aggiungere le colonne seguenti:  
    ...
    - **ID dispositivo Azure Active Directory**: identificatore univoco del dispositivo nel Azure Active Directory.  

    - **Dettagli dell'errore di conformità**: dettagli del messaggio di errore quando il processo non è corretto. Se questa colonna è vuota, significa che non sono stati rilevati errori ed è stato segnalato lo stato di conformità.  

    - **Posizione dell'errore di conformità**: ulteriori informazioni sul punto in cui la conformità non è riuscita. Se questa colonna è vuota, significa che non sono stati rilevati errori ed è stato segnalato lo stato di conformità. Esempi di posizione in cui il processo di conformità può restituire un errore:  
    ...
    - **Ora della valutazione della conformità**: ora dell'ultima verifica della conformità.  

    - **Ora del set di conformità**: ora dell'ultimo aggiornamento della conformità per Azure Active Directory.  

    - **Conforme all'accesso condizionale**: indipendentemente dal fatto che il computer sia conforme ai criteri di accesso condizionale.  
...
2. Nell'area di lavoro **Monitoraggio** passare a **Panoramica** > **Impostazioni di conformità** > **Criteri di conformità**. Vengono visualizzati i grafici seguenti:  
    ...
    - **Conformità complessiva del dispositivo**: Mostra la conformità complessiva dei dispositivi per tutti i criteri di conformità.  

    - **Motivi principali per la mancata conformità**: Mostra i criteri principali per i quali i dispositivi non sono conformi.  
...
- **Richiedi aggiornamenti automatici**: è possibile richiedere ai dispositivi con Windows 8.1 o versioni successive di installare automaticamente gli aggiornamenti ed è possibile specificare la classe degli aggiornamenti.  
...
- Il **profilo di posta elettronica deve essere gestito da Intune**: quando si imposta questa opzione su **Sì**, il dispositivo deve usare il profilo di posta elettronica distribuito al dispositivo. Il dispositivo viene considerato non conforme se il profilo di posta elettronica non viene distribuito allo stesso gruppo di utenti a cui si applicano i criteri di conformità.  
    ...
    È anche non conforme se l'utente ha già configurato un account di posta elettronica nel dispositivo che corrisponde al profilo di posta elettronica di Intune distribuito nel dispositivo. In questo caso, Intune non può sovrascrivere il profilo con provisioning dell'utente, quindi non può gestirlo. L'utente può rendere conforme il dispositivo rimuovendo l'impostazione di posta elettronica esistente, consentendo quindi a Intune di installare il profilo di posta elettronica gestito.  
...
- **Richiedi che i dispositivi siano segnalati come integri**: è possibile impostare una regola per richiedere che i dispositivi Windows 10 vengano segnalati come integri in criteri di conformità nuovi o esistenti. Se si abilita questa impostazione, i dispositivi Windows 10 vengono valutati tramite il servizio di attestazione dell'integrità per i punti dati seguenti:  
    ...
    - **BitLocker è abilitato**: Quando BitLocker è attivato, il dispositivo può proteggere i dati archiviati nell'unità da accessi non autorizzati, quando il sistema è disattivato o passa alla modalità di ibernazione.  
    ...
    **Supportato in**:  
    ...
    > [!NOTE]  
    > A partire da Configuration Manager 1802, Software Center Mostra l'elemento di attestazione dell'integrità il cui dispositivo non è conforme. <!--1235616-->   
    > ![Attestazione dell'integrità del dispositivo conformità del dispositivo in Software Center](./media/Software-Center-noncompliant.PNG)  
