...
### <a name="BKMK_CreatingDriverPackages"></a> Creare un pacchetto driver  
...
Per altre informazioni sugli account di accesso al pacchetto, vedere [Account usati in Configuration Manager](/sccm/core/plan-design/hierarchy/accounts).
...
A partire dalla versione 1906, usare questi attributi per la pre-cache del contenuto di un client. Per altre informazioni, vedere [Configurare la pre-cache del contenuto](/sccm/osd/deploy-use/configure-precache-content).<!--4224642-->  
...
3. Nella pagina **Trova driver** specificare le opzioni seguenti:  
    ...
    - **Importa tutti i driver nel percorso di rete seguente (UNC)** : per importare tutti i driver di dispositivo in una cartella specifica, specificare il percorso di rete. Ad esempio: `\\servername\share\folder`.  
...
## <a name="BKMK_TSDrivers"></a> Usare le sequenze di attività per installare i driver  
...
## <a name="BKMK_DriverReports"></a> Report per i driver  

È possibile utilizzare diversi report nella categoria di report **Gestione driver** per determinare informazioni generali sui driver di dispositivo nel catalogo. Per altre informazioni sui report, vedere [Creazione di report](/sccm/core/servers/manage/reporting).
