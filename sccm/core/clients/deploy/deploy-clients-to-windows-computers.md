...
- Usare l'Installazione guidata push client per installare il client di Configuration Manager, utilizzabile per eseguire [query](/sccm/core/servers/manage/introduction-to-queries) sul risultato. L'installazione avrà esito positivo solo se uno degli elementi restituiti dalla query è l'attributo **ResourceID** dalla classe **Risorse di sistema**.
...
> [!IMPORTANT]  
>  Il push client avrà esito positivo solo se tutti i prerequisiti sono soddisfatti. Per altre informazioni, vedere [Dipendenze del metodo di installazione](/sccm/core/clients/deploy/prerequisites-for-deploying-clients-to-windows-computers#installation-method-dependencies).
...
Questa procedura è valida per un client tradizionale connesso a una rete Intranet. Usa i metodi di autenticazione client tradizionali. Per garantire che il dispositivo rimanga in uno stato gestito dopo l'installazione del client, il dispositivo deve essere nella rete Intranet e all'interno di un limite del sito di Configuration Manager.  
...
> [!NOTE]  
> Per impostazione predefinita, in alcune versioni di Configuration Manager, il dispositivo annulla la registrazione da Intune dopo aver installato il software client.
> 
> A partire dalla versione 1710, la registrazione dei client con Intune non viene annullata. È possibile usare sia il client di Configuration Manager sia la registrazione MDM nello stesso momento. Per altre informazioni, vedere la [panoramica sulla co-gestione](/sccm/comanage/overview).  


###  <a name="install-clients-by-using-intune"></a>Installare i client con Intune  

1. In Intune [aggiungere un'app line-of-business](https://docs.microsoft.com/intune/lob-apps-windows) di Windows contenente il file di installazione client di Configuration Manager CCMSetup.msi. Questo file è disponibile nella cartella seguente nel server del sito: `<ConfigMgr installation directory>\bin\i386`.  
...
> [!IMPORTANT]  
> Quando si usa la sequenza di attività di Configuration Manager per distribuire un'immagine del sistema operativo, il passaggio [Prepara Client ConfigMgr](/sccm/osd/understand/task-sequence-steps#BKMK_PrepareConfigMgrClientforCapture) rimuove completamente il client di Configuration Manager.  
...
> [!NOTE]  
> Questa sezione non si applica ai client che usano un [gateway di gestione cloud](/sccm/core/clients/manage/plan-cloud-management-gateway). Per installare i client basati su Internet tramite un gateway di gestione cloud, vedere [Installare e assegnare client di Configuration Manager in dispositivi Windows 10 usando Azure AD per l'autenticazione](/sccm/core/clients/deploy/deploy-clients-cmg-azure).  

Quando il sito di Configuration Manager supporta la [gestione client basata su Internet](/sccm/core/clients/manage/plan-internet-based-client-management) per client che si trovano a volte in una rete Intranet e a volte su Internet, sono disponibili due opzioni per l'installazione dei client nella rete Intranet:  
...
1.  Aprire un editor di testo come il Blocco note.  

2.  Copiare e inserire il codice VBScript seguente nel file. Sostituire `mp.contoso.com` con l'FQDN Internet del punto di gestione basato su Internet.  
...
5.  Usare cscript.exe per eseguire lo script nei computer client. Usare uno dei metodi seguenti:  
...
-   Si usano le impostazioni di Criteri di gruppo o i metodi di installazione client basati sull'aggiornamento software. Lo schema di Active Directory non è stato esteso a Configuration Manager.  
