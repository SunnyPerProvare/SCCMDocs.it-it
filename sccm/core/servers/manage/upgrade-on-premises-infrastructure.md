...
- Per eseguire l'*aggiornamento* a Configuration Manager Current Branch da una versione precedente, vedere [Eseguire l'aggiornamento a Configuration Manager](/sccm/core/servers/deploy/install/upgrade-to-configuration-manager).  
...
- Se si sta aggiornando il sistema operativo del server del sito, verificare che la [replica basata su file](/sccm/core/plan-design/hierarchy/file-based-replication) sia integra per il sito. Controllare tutte le cartelle di posta in arrivo per verificare la presenza di un backlog sia nei siti di origine che in quelli di destinazione. Se sono presenti molti processi di replica bloccati o in sospeso, attendere che vengano cancellati.<!-- SCCMDocs#1792 -->
...
- Assicurarsi che ogni server che ospita un ruolo del sistema del sito continui a soddisfare tutti i [prerequisiti](/sccm/core/plan-design/configs/site-and-site-system-prerequisites). Potrebbe essere necessario ad esempio reinstallare BITS, WSUS, o configurare impostazioni specifiche per IIS.  
...
- Aggiornamento sul posto di Windows da una versione supportata di Windows 10. Per altre informazioni, vedere [Aggiornare Windows alla versione più recente](/sccm/osd/deploy-use/upgrade-windows-to-the-latest-version).  
...
Per informazioni sulle versioni di SQL Server supportate da Configuration Manager, vedere [Supporto per le versioni di SQL Server](/sccm/core/plan-design/configs/support-for-sql-server-versions).  
