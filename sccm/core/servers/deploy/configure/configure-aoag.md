...
1. Usare il comando seguente per arrestare il sito di Configuration Manager:
    ...
    Per altre informazioni, vedere [Strumento di manutenzione gerarchia](/sccm/core/servers/manage/hierarchy-maintenance-tool-preinst.exe).
...
5. Controllare la configurazione in ogni replica:
    ...
    2. Eseguire lo [script di verifica](/sccm/core/servers/deploy/configure/sql-server-alwayson-for-a-highly-available-site-database#prerequisites) per assicurarsi che il database del sito in ogni replica sia configurato correttamente.
...
1. Arrestare il sito primario attivo per impedire altre operazioni di scrittura nel database del sito. A questo scopo, usare lo [strumento di manutenzione gerarchia](/sccm/core/servers/manage/hierarchy-maintenance-tool-preinst.exe): `preinst.exe /stopsite`
