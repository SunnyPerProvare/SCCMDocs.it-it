...
| 0x00D00001 | L'installazione di cache in rete di Ottimizzazione recapito può essere eseguita solo se Internet Information Services (IIS) è stato installato |
| 0x00D00002 | L'installazione di cache in rete di Ottimizzazione recapito può essere eseguita solo se nel server è presente un sito Web predefinito |
| 0x00D00003 | Non è possibile installare cache in rete di Ottimizzazione recapito se ARR (Application Request Routing) è già installato |
| 0x00D00004 | L'installazione di cache in rete di Ottimizzazione recapito può essere eseguita solo se ARR (Application Request Routing) è stato installato dallo script Install.ps1 |
| 0x00D00005 | L'installazione di cache in rete di Ottimizzazione recapito richiede che la sessione di PowerShell sia in esecuzione come amministratore |
| 0x00D00006 | L'installazione di cache in rete di Ottimizzazione recapito può essere eseguita solo da un ambiente PowerShell a 64 bit |
| 0x00D00007 | L'installazione di cache in rete di Ottimizzazione recapito può essere eseguita solo in Windows Server |
...
| 0x00D0001E | Non è possibile installare cache in rete di Ottimizzazione recapito se il sito Web predefinito non si trova sulla porta 80 |
