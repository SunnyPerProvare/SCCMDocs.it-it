---
title: Eseguire l'aggiornamento a System Center Configuration Manager | System Center Configuration Manager
...
##  <a name="a-namebkmkpatha-in-place-upgrade-paths"></a><a name="bkmk_path"></a> Percorsi di aggiornamento sul posto  
 **È possibile aggiornare i prodotti seguenti a una versione con licenza completa di System Center Configuration Manager versione 1511:**  
...
**È possibile aggiornare i prodotti seguenti a una versione con licenza completa di System Center Configuration Manager versione 1606:**
...
> [!TIP]  
>  Quando si esegue l'aggiornamento da una versione di System Center 2012 Configuration Manager, il processo di aggiornamento potrebbe risultare semplificato. Per altre informazioni, vedere  
>   
...
##  <a name="a-namebkmkchecklista-upgrade-checklists"></a><a name="bkmk_checklist"></a> Eseguire l'aggiornamento degli elenchi di controllo  
...
**Installare tutti gli aggiornamenti critici disponibili** per i sistemi operativi sui computer che ospitano il sito, sul server del database del sito e per i ruoli del sistema del sito remoto:  
...
**Disabilitare le repliche di database** per i punti di gestione presso i siti primari:  
...
Prima di aggiornare un sito, è possibile eseguire il **controllo dei prerequisiti** in modo indipendente dal programma di installazione per convalidare che il sito soddisfa i prerequisiti. Quando si aggiorna il sito, il controllo dei prerequisiti viene nuovamente eseguito.  

Per altre informazioni, vedere [Prerequisite Checker](/sccm/core/servers/deploy/install/prerequisite-checker) e [List of Prerequisite Checks for System Center Configuration Manager](/sccm/core/servers/deploy/install/list-of-prerequisite-checks)  

**Scaricare i file di prerequisiti e i file ridistribuibili** per System Center Configuration Manager:  
...
Per informazioni, vedere [Setup Downloader](/sccm/core/servers/deploy/install/setup-downloader).  
...
**Creare un backup** del database del sito presso il sito di amministrazione centrale e presso i siti primari:  
...
Vedere [Backup and recovery for System Center Configuration Manager](../../../../protect/understand/backup-and-recovery.md).  
...
**Testare il processo di aggiornamento del database** su una copia del backup più recente del database del sito:  
...
**Riavviare il server del sito e ogni computer che ospita un ruolo del sistema del sito** per assicurarsi che non ci siano azioni in sospeso relative a un'installazione recente di aggiornamenti o relative a prerequisiti:  

Processo interno specifico della società.  
...
**Riconfigurare le repliche di database** presso i punti di gestione nei siti primari:  
...
##  <a name="a-namebkmkconsiderationsa-considerations-for-upgrading"></a><a name="bkmk_considerations"></a> Considerazioni sull'aggiornamento  
**Azioni automatiche**: quando si esegue l'aggiornamento a System Center Configuration Manager, le azioni seguenti sono automatiche:  
...
**Azioni manuali per l'utente amministratore dopo un aggiornamento** : quando si esegue l'aggiornamento di un sito, verificare che vengano eseguite le seguenti azioni:  
...
**Azioni che influiscono su configurazioni e impostazioni**: quando un sito viene aggiornato a System Center Configuration Manager, alcune configurazioni e impostazioni non vengono mantenute dopo l'aggiornamento oppure vengono impostate su una nuova configurazione predefinita. La tabella seguente include le impostazioni che non vengono mantenute o che vengono modificate e fornisce dettagli per la relativa pianificazione durante l'aggiornamento di un sito:  
...
##  <a name="a-namebkmktesta-test-the-site-database-upgrade"></a><a name="bkmk_test"></a> Testare l'aggiornamento del database del sito  
...
>   
> Entrambi i messaggi possono essere tranquillamente ignorati durante il test di un aggiornamento del database perché non indicano un errore o un problema con l'aggiornamento di prova. Indicano piuttosto che, durante l'aggiornamento effettivo, i dati del gruppo di replica di database **Cloud** potrebbero essere sincronizzati con Microsoft Intune.  
...
##  <a name="a-namebkmkupgradea-upgrade-sites"></a><a name="bkmk_upgrade"></a> Aggiornare i siti  
...
1.  Verificare che l'utente che esegue l'installazione disponga dei seguenti privilegi di protezione:  
    ...
    -   Diritti di amministratore locale sul server di database del sito remoto per il sito, se è remoto.  
...
**Problemi noti:**  
Siti secondari:
- Durante l'aggiornamento alla versione 1511:  
Affinché i client nei siti secondari possano trovare il punto di gestione dal sito secondario, vale a dire dal punto di gestione proxy, aggiungere manualmente il punto di gestione ai gruppi di limiti che includono anche i punti di distribuzione nel sito secondario.

- Durante l'aggiornamento alla versione 1606 o successiva:  
  I punti di gestione proxy vengono aggiunti automaticamente ai gruppi di limiti che includono i punti di distribuzione nel sito secondario.
